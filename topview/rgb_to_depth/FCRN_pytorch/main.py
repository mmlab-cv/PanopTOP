...
from dataloaders import kitti_dataloader, nyu_dataloader, panoptic_dataloader
...
def create_loaders(args):
    traindir = get_if_exists("train")
    valdir = get_if_exists("validation")
    testdir = get_if_exists("test")

    sampler, train_set, val_set, test_set = get_sets(args, traindir, valdir, testdir)
    batch_size = 3
    ...
    val_loader = torch.utils.data.DataLoader(
        val_set, batch_size=batch_size, shuffle=False, num_workers=args.workers, pin_memory=True)

    test_loader = torch.utils.data.DataLoader(
        test_set, batch_size=batch_size, shuffle=False, num_workers=args.workers, pin_memory=True)
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        result, img_merge = test(val_loader, model, epoch, logger)  # evaluate on validation set
...
def train(train_loader, model, criterion, optimizer, epoch, logger):
    ...
    for i, (input, target) in enumerate(train_loader):
        im = input[0].cpu().numpy().transpose((1,2,0))
        print("input shape", im.shape)

        utils.save_image(im, "imgs/im.png")
...
# validation
def test(test_loader, model, epoch, logger):
    average_meter = AverageMeter()
    model.eval()  # switch to evaluate mode
    end = time.time()

    skip = len(test_loader) // 3  # save images every skip iters
    ...
    for i, (input, target) in enumerate(test_loader):
        ...
        if i == 0:
            ...
            filename = f"image_results/{str(epoch)}.png"
