...
class V2VVoxelization(object):
    def __init__(self, augmentation=True):
        # cubic_size is a size within which input data is scattered, 
        # cubic_size == 2 means, that everything will be in cage "-1 / +1"
        self.cubic_size = 2
    ...
    def __call__(self, sample):
        ...
        if self.augmentation:
            np.save("tmp/train", input)
        else:
            np.save("tmp/val", input)
        '''
